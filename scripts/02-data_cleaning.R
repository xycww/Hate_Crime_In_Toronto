...
cleaned_data <- raw_data %>%
  mutate(
    MENTAL_OR_PHYSICAL_DISABILITY = ifelse(
      MENTAL_OR_PHYSICAL_DISABILITY == "NO",
      "None",
  ...
  select(
    -NEIGHBOURHOOD_140,-HOOD_140,-NEIGHBOURHOOD_158,-HOOD_158,-DIVISION,-REPORTED_TIME,
...
na_checks <- sapply(cleaned_data, function(x)
  sum(is.na(x)))
