...
# 70% None, 10% of women, and the others share the rest of proportion
gender_bias_probs <- c(0.05, 0.7, 0.05, 0.1, 0.05, 0.05)

race_bias_levels <- unique(simulated_data$RACE_BIAS)
# 40% None, 30% of Black, and the others share the rest of proportion, (Asians tend to share more)
race_bias_probs <- c(
  0.01,
  0.01,
  0.01,
...
# 40% None, 40% of Jewish, and the others share the rest of proportion
...
# 60% None, and the others share the rest of proportion. Asian and Indian will share more.
ethnicity_bias_probs <- c(
  0.01,
  0.01,
  0.05,
  0.01,
...
# 60% None, and the others share the rest of proportion equally.
sexual_orientation_bias_probs <- c(0.1, 0.1, 0.1, 0.6, 0.1)

language_bias_levels <- unique(simulated_data$LANGUAGE_BIAS)
# 97% None, and the others share the rest of proportion equally.
...
simulated_data <-
  tibble(
    ...
    SEXUAL_ORIENTATION_BIAS = sample(
      sexual_orientation_bias_levels,
      n,
      replace = TRUE,
      prob = sexual_orientation_bias_probs
    ),
    ...
    MENTAL_OR_PHYSICAL_DISABILITY = sample(
      mental_physical_disability_levels,
      n,
      replace = TRUE,
      prob = mental_physical_disability_probs
    ),
